	...
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>,
{
	...
	pub fn bare_instantiate(
		...
		salt: Vec<u8>,
		debug: bool,
	) -> AccountInstantiateResult<T::AccountId, BalanceOf<T>> {
		let mut debug_message = if debug { Some(Vec::new()) } else { None };
...
sp_api::decl_runtime_apis! {
	...
	#[api_version(2)]
	pub trait AccountAbstractionApi<AccountId, Balance, BlockNumber, Hash> where
		AccountId: Codec,
		Balance: Codec,
		BlockNumber: Codec,
		Hash: Codec,
	{
		...
		fn instantiate(
			...
			data: Vec<u8>,
			salt: Vec<u8>,
		) -> AccountInstantiateResult<AccountId, Balance>;
