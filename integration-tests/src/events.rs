...
// use crate::{cmd::runtime_api::api::contracts::events::ContractEmitted, DEFAULT_KEY_COL_WIDTH};
// use colored::Colorize as _;
// use contract_build::Verbosity;
use contract_transcode::{ContractMessageTranscoder, Hex, TranscoderBuilder, Value};

use anyhow::Result;
use sp_runtime::scale_info::form::PortableForm;
// use scale_info::form::PortableForm;
use std::str::FromStr;
use subxt::{self, blocks::ExtrinsicEvents, events::StaticEvent};

use crate::{runtime::account_abstraction::events::ContractEmitted, DefaultConfig};
...
impl DisplayEvents {
		...
		for event in result.iter() {
			...
			for field_metadata in event_fields {
				...
				) && field_metadata.name == Some("data".to_string())
				{
					...
					let field = contract_event_data_field(transcoder, field_metadata, event_data)?;
					event_entry.fields.push(field);
				} else {
					let field_name =
						field_metadata.name.as_ref().map(|s| s.to_string()).unwrap_or_else(|| {
	...
	// 	let mut out =
	// 		format!("{:>width$}\n", "Events".bright_purple().bold(), width = DEFAULT_KEY_COL_WIDTH);
	// 	for event in &self.0 {
	// 		let _ = writeln!(
	// 			out,
	// 			"{:>width$} {} ➜ {}",
	...
	// 						value =
	// 							BalanceVariant::from(balance, Some(token_metadata))?.to_string();
	// 					}
	// 				}
	// 				let _ = writeln!(
	// 					out,
